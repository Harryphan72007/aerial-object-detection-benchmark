...
from src.models.vmamba_frcnn.trainer import VMambaSharedTrainer
...
def test_vmamba_batch_step_resume_and_legacy_evaluator(tmp_path: Path) -> None:
    optimizer = _Component()
    scheduler = _Component()
    state = TrainingState()
    backward: list[float] = []
    trainer = VMambaSharedTrainer(accumulation_steps=2)
    ...
    checkpoint = tmp_path / "vmamba_state.json"
    ...
    identity = ArtifactIdentity(
        "vmamba-smoke", "faster_rcnn_vmamba_t", "State Space Model", "2class", 640, 42
    )
    ...
    assert metrics["per_class_detailed"]["vehicle"]["true_positives"] == 1
